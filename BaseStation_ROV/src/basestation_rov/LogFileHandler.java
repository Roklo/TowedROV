...
public class LogFileHandler implements Runnable
{
    ...
    public LogFileHandler(DataHandler dh)
    {
        this.dh = dh;
    }
...
//                    }
    ...
    private void logPhotoPosition(BufferedWriter bw)
    {
        try
        {
            double shipsLatitude = dh.get_Latitude();
            double shipsLontitude = dh.get_Longitude();
            double rovHeadingRelToShip = dh.get_Heading() - 180;
            ...
            rovLatitude = dh.get_Latitude() - (adjustedCoordinateRovOffset * rovHeadingRelToShipSinRest);
            if (rovHeadingRelToShipSinRest != 0)
            {
                rovLongtitude = dh.get_Longitude() + (adjustedCoordinateRovOffset * rovHeadingRelToShipSin);
            } else
            {
                rovLongtitude = dh.get_Longitude();
            }

            photoLocationTrack = "";
            photoLocationTrack = photoLocationNumb + ","
                    + dh.get_Latitude() + "," + dh.get_Longitude() + ","
                    + dh.get_Speed() + "," + dh.rovDepth;
    ...
    private void logShipPosition(BufferedWriter bw)
    {
        ...
        try
        {
            ...
            shipTrack = shipTrackPointNumb + ","
                    + dh.get_Latitude() + "," + dh.get_Longitude() + ","
                    + dh.get_Speed() + "," + dh.rovDepth + "," + dh.get_Heading();
